...
export function Gallery({
  ...
  items: Array<string | { src: string; label?: string }>;
  title?: string;
}) {
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        {items.map((item, i) => {
          ...
          const aspect =
            i === 0 ? "aspect-[16/10] col-span-2 sm:col-span-2 sm:row-span-2" : "aspect-[4/3]";

          return <ImagePlaceholder key={label + i} label={label} src={src} aspect={aspect} />;
        })}
      </div>
    </div>
  );
